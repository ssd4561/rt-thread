 ...
 * @LastEditTime: 2021-04-23 08:53:12
...
void FI2C_irqHandler4Fifo(void *pArgs)
{
    ...
    if (I2C_IRQ_RX_FULL & RegVal)
    {
        ...
        //FT_I2C_DEBUG_I("I2C_IRQ_RX_FULL bytes to rx %d", BytesToRx);
               ...
               (0 < BytesToRx))
        {
            ...
            // FT_I2C_DEBUG_I("read 0x%x with idx %d, len %d remain %d",
            //                pDev->RxBuf.BytePtr[pDev->RxBuf.CurIndex],
            //                pDev->RxBuf.CurIndex,
            //                pDev->RxBuf.DataLength,
            //                BytesToRx);
            pDev->RxBuf.CurIndex++;
            BytesToRx--;
            pDev->DelayHandle(10);
            ...
            else
            {
                ...
                pDev->DelayHandle(10);
                FI2C_setIrq(pDev, I2C_IRQ_RX_FULL, FALSE);
                //FT_I2C_DEBUG_I("stop and disable I2C_IRQ_RX_FULL");
            }
        }
    }
    else if (I2C_IRQ_TX_EMPTY & RegVal)
    {
        IrqType = I2C_IRQ_TYPE_TX_COMPLETE;
        // FT_I2C_DEBUG_I("I2C_IRQ_TX_EMPTY idx %d, len %d",
        //             pDev->TxBuf.CurIndex,
        //             pDev->TxBuf.DataLength);
        if (pDev->TxBuf.CurIndex == pDev->TxBuf.DataLength)
        {
            FI2C_SEND_TX_STOP_CMD(pDev);
            pDev->DelayHandle(10);
            //FT_I2C_DEBUG_I("stop and disable I2C_IRQ_TX_EMPTY");
            FI2C_setIrq(pDev, I2C_IRQ_TX_EMPTY, FALSE);
        }
        else
        {
            //FT_I2C_DEBUG_I("TX 0x%x", pDev->TxBuf.BytePtr[pDev->TxBuf.CurIndex]);
