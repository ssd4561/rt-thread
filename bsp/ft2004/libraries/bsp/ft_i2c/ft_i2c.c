                     ...
                     FT_INOUT FI2C_t *pDev)
{
    ...
    // FT_I2C_DEBUG_I("set rx thres 0x%x, tx thres 0x%x",
    //             FT_MIN(pDev->Config.RxThres, RxDepth),
    //             FT_MIN(pDev->Config.TxThres, TxDepth));
...
u32 FI2C_writeByByte(FT_IN u32 len, FT_IN u8 *pI2cBuf, FT_IN u8 PageAddr, FT_INOUT FI2C_t *pDev)
{
    ...
    // FT_I2C_DEBUG_I("get rx thres 0x%x, tx thres 0x%x",
    //                 FI2C_GET_TXFLR(pDev),
    //                 FI2C_GET_RXFLR(pDev));
